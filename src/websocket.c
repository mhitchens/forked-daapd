...
process_notify_request(struct ws_session_data_notify *session_data, void *in, size_t len)
{
  ...
  if (jerr != json_tokener_success)
    {
      DPRINTF(E_LOG, L_WS, "Failed to parse incoming request: %s\n", json_tokener_error_desc(jerr));
      json_tokener_free(tokener);
      return -1;
    }

  DPRINTF(E_DBG, L_WS, "notify callback request: %s\n", json_object_to_json_string(request));
	  ...
	  if (json_object_get_type(item) == json_type_string)
	    {
	      ...
	      DPRINTF(E_DBG, L_WS, "notify callback event received: %s\n", event_type);
...
callback_notify(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len)
{
  ...
  DPRINTF(E_DBG, L_WS, "notify callback reason: %d\n", reason);
  switch (reason)
    {
...
websocket_init(void)
{
  ...
  if (websocket_port <= 0)
    {
      DPRINTF(E_LOG, L_WS, "Websocket disabled. To enable it, set websocket_port in config to a valid port number.\n");
      return 0;
    }
  ...
  if (context == NULL)
    {
      DPRINTF(E_LOG, L_WS, "Failed to create websocket context\n");
      return -1;
    }
  ...
  if (ret < 0)
    {
      DPRINTF(E_LOG, L_WS, "Could not spawn websocket thread: %s\n", strerror(errno));
