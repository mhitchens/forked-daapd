...
jsonapi_reply_config(struct evhttp_request *req, struct evbuffer *evbuf, char *uri, struct evkeyvalq *query)
{
...
#ifdef ITUNES
  json_object_array_add(buildopts, json_object_new_string("iTunes XML"));
#endif
#ifdef SPOTIFY
  json_object_array_add(buildopts, json_object_new_string("Spotify"));
#endif
#ifdef LASTFM
  json_object_array_add(buildopts, json_object_new_string("LastFM"));
#endif
#ifdef CHROMECAST
  json_object_array_add(buildopts, json_object_new_string("Chromecast"));
#endif
#ifdef MPD
  json_object_array_add(buildopts, json_object_new_string("MPD"));
#endif
#ifdef RAOP_VERIFICATION
  json_object_array_add(buildopts, json_object_new_string("Device verification"));
#endif
#ifdef WEBSOCKET
  json_object_array_add(buildopts, json_object_new_string("Websocket"));
#endif
#ifdef HAVE_ALSA
  json_object_array_add(buildopts, json_object_new_string("ALSA"));
#endif
#ifdef HAVE_LIBPULSE
  json_object_array_add(buildopts, json_object_new_string("Pulseaudio"));
#endif
  ...
  if (ret < 0)
    {
      DPRINTF(E_LOG, L_HTTPD, "config: Couldn't add config data to response buffer.\n");
      return -1;
    }
...
jsonapi_reply_library(struct evhttp_request *req, struct evbuffer *evbuf, char *uri, struct evkeyvalq *query)
{
  ...
  if (ret < 0)
    {
      DPRINTF(E_LOG, L_HTTPD, "library: failed to get file count info\n");
      return -1;
    }
  ...
  if (ret < 0)
    {
      DPRINTF(E_LOG, L_HTTPD, "library: Couldn't add library information data to response buffer.\n");
      return -1;
    }
...
jsonapi_reply_spotify(struct evhttp_request *req, struct evbuffer *evbuf, char *uri, struct evkeyvalq *query)
{
  ...
  if (!uri)
    {
      DPRINTF(E_LOG, L_HTTPD, "Cannot display Spotify oauth interface (http_form_uriencode() failed)\n");
    }
  else
    {
  ...
  if (ret < 0)
    {
      DPRINTF(E_LOG, L_HTTPD, "spotify: Couldn't add spotify information data to response buffer.\n");
      return -1;
    }
...
jsonapi_reply_spotify_login(struct evhttp_request *req, struct evbuffer *evbuf, char *uri, struct evkeyvalq *query)
{
  ...
  DPRINTF(E_DBG, L_HTTPD, "Received spotify login request\n");
  ...
  if (!request)
    {
      DPRINTF(E_LOG, L_HTTPD, "Failed to parse incoming request\n");
      return -1;
    }
  ...
  if (user && strlen(user) > 0 && password && strlen(password) > 0)
    ...
    {
      DPRINTF(E_LOG, L_HTTPD, "No user or password in spotify login post request\n");
  ...
  if (ret < 0)
    {
      DPRINTF(E_LOG, L_HTTPD, "spotify: Couldn't add spotify login data to response buffer.\n");
      return -1;
    }

#else
  DPRINTF(E_LOG, L_HTTPD, "Received spotify login request but was not compiled with enable-spotify\n");
#endif
...
pairing_kickoff(struct evhttp_request* req)
{
  ...
  DPRINTF(E_DBG, L_HTTPD, "Received pairing post request: %s\n", json_object_to_json_string(request));

  message = jparse_str_from_obj(request, "pin");
  if (message)
    remote_pairing_kickoff((char **)&message);
  else
    DPRINTF(E_LOG, L_HTTPD, "Missing pin in request body: %s\n", json_object_to_json_string(request));
...
pairing_get(struct evbuffer *evbuf)
{
  ...
  if (ret < 0)
    {
      DPRINTF(E_LOG, L_HTTPD, "pairing: Couldn't add pairing information data to response buffer.\n");
      return -1;
    }
...
jsonapi_request(struct evhttp_request *req)
{
  ...
  if (!httpd_admin_check_auth(req))
    {
      DPRINTF(E_DBG, L_HTTPD, "JSON api request denied;\n");
      return;
    }
  ...
  DPRINTF(E_DBG, L_HTTPD, "Web admin request: %s\n", full_uri);
  ...
  if (handler < 0)
    {
      DPRINTF(E_LOG, L_HTTPD, "Unrecognized web admin request\n");
  ...
  if (!evbuf)
    {
      DPRINTF(E_LOG, L_HTTPD, "Could not allocate evbuffer for Web Admin reply\n");
...
jsonapi_init(void)
{
  ...
  for (i = 0; adm_handlers[i].handler; i++)
      ...
      if (ret != 0)
	{
	  ...
	  DPRINTF(E_FATAL, L_HTTPD, "Admin web interface init failed; regexp error: %s\n", buf);
	  return -1;
	}
    }
